...
test('kk: квиз КРС — условные вопросы на казахском', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Импорттық па, әлде отандық мал ма?' })).toBeVisible();
  await page.getByRole('button', { name: /Импорттық асыл тұқымды мал/ }).click();
